...
bot = telebot.TeleBot('7378613818:AAEMrniTql_qin6--ZkYmH0x5ntCPShL4zA')
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 0:
                response = "You Are On Cooldown . Please Wait sec Before Running The /bgmi Command Again."
                bot.reply_to(message, response)
                return
...
def welcome_start(message):
    ...
    response = f'''👋🏻Welcome to VIP Ddos Attack in BGMI, {user_name}! Feel Free to Explore.
🤖Try To Run This Command : /help 
✅Join :- BUY @I_AM_OWNEER'''
    bot.reply_to(message, response)
